class bginfo::file {
  ...
  $bginfo::displayed_facts.each |String $fact| {
    ...
    if $fact_parts[0] == 'trusted' {
      $trusted_parts = $fact_parts[1, $fact_parts.length - 1]
      $fact_value    = dig($trusted, *$trusted_parts)
      notify { "This was trusted parts ${trusted_parts}": }
    } else {
  ...
  file { "${bgi_path}${bgi_dir}/test.txt":
    ...
    source  => epp('bginfo/bginfo.epp', { 'the_facts' => $bginfo::displayed_facts }),
    require => File["${bgi_path}${bgi_dir}"],
  }
}
